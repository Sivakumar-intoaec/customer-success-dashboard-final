...
import { PortfolioAnalyticsBody } from '../types';
    ...
    <div className="flex flex-col h-full justify-between">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sky-650 text-sky-655 text-sky-600 text-[11px] font-bold tracking-widest uppercase">IntoAEC</p>
              <h1 className="text-2xl font-black text-slate-800 leading-tight">Customer Success Review</h1>
            </div>
          </div>
        ...
        <div className="text-right hidden sm:block">
          <div className="text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-1">Portfolio size</div>
          <div className="text-4xl font-black text-slate-800">{paidOrgsCount || s.totalAccounts}</div>
          <div className="text-xs text-slate-400 font-bold">Paid active accounts</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-6">
        {[
          ...
          { label: 'Need CS Focus', value: s.accountsNeedingAttention, suffix: 'accounts', color: '#b45309', icon: AlertTriangle },
          { label: 'Inactive Risk', value: s.churnRiskOrgs, suffix: 'orgs', color: '#b91c1c', icon: Flame },
          { label: 'Daily Engagement', value: `${Math.round((s.avgStickiness || 0) * 100)}%`, suffix: 'DAU/MAU', color: '#0f766e', icon: Activity },
        ].map((kpi, i) => {
          const Icon = kpi.icon;
          return (
            <div key={i} className="rounded-2xl p-4 border bg-white border-slate-205 border-slate-200 shadow-sm relative overflow-hidden">
              <div className="absolute -top-4 -right-4 w-20 h-20 rounded-full blur-2xl opacity-10 pointer-events-none"
                style={{ background: kpi.color }} />
              <div className="p-2 rounded-xl w-fit mb-2" style={{ background: `${kpi.color}10` }}>
      ...
      <div>
        ...
        <div className="flex h-5 w-full rounded-full overflow-hidden gap-0.5 bg-slate-100 p-0.5 border border-slate-200">
          {s.distribution.healthy > 0 && (
            <div className="flex items-center justify-center text-[10px] font-bold text-white transition-all rounded-l-full"
          ...
          {s.distribution.critical > 0 && (
            <div className="flex items-center justify-center text-[10px] font-bold text-white rounded-r-full"
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Pie */}
        <div className="flex flex-col items-center justify-center bg-slate-50/50 rounded-2xl border border-slate-100">
          <div className="relative w-60 h-60">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData} cx="50%" cy="50%" innerRadius={70} outerRadius={100} paddingAngle={3} dataKey="value" strokeWidth={0}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-3xl font-black text-slate-800">{Math.round(s.avgHealthScore)}</span>
        ...
        <div className="flex flex-col justify-center gap-4">
            ...
            <div key={row.label}>
              <div className="flex items-center justify-between text-xs mb-1.5 font-bold">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-base" style={{ color: row.color }}>{row.count}</span>
                  <span className="text-slate-400 font-normal">({row.pct}%)</span>
                </div>
              </div>
              <div className="h-2 rounded-full overflow-hidden bg-slate-100 border border-slate-200/50">
          ...
          <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 gap-3">
            {[
              { label: 'Improving', value: s.trends?.improving ?? 0, color: '#047857' },
              { label: 'Stable', value: s.trends?.stable ?? 0, color: '#64748b' },
              { label: 'Declining', value: s.trends?.declining ?? 0, color: '#b91c1c' },
              { label: 'Automation score', value: `${Math.round(s.avgAutomationScore || 0)}%`, color: '#6d28d9' },
            ].map((t) => (
              <div key={t.label} className="text-center p-2 rounded-xl bg-slate-50 border border-slate-200/65">
                <div className="text-lg font-black" style={{ color: t.color }}>{t.value}</div>
                <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">{t.label}</div>
              </div>
            ))}
          </div>
        </div>
...
function SlideTrend({ data }: { data: PortfolioAnalyticsBody }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 bg-slate-50/50 rounded-2xl border border-slate-100 p-3">
        {trend.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trend} margin={{ top: 10, right: 20, left: -10, bottom: 5 }}>
              ...
              <RechartsTooltip contentStyle={tooltipStyle}
                formatter={(v: number, n: string) => [n === 'avgHealthScore' ? `${v}/100` : `${v}%`, n === 'avgHealthScore' ? 'Avg Health' : 'Engagement']} />
              <Area type="monotone" dataKey="avgHealthScore" stroke="#0ea5e9" strokeWidth={3}
                fill="url(#pHealthGrad)" dot={{ r: 3, fill: '#0ea5e9', strokeWidth: 0 }}
          ...
          <div className="h-full flex flex-col items-center justify-center gap-4">
            <div className="p-6 rounded-3xl bg-white border border-slate-200/50 shadow-sm">
              <TrendingUp className="w-12 h-12 text-slate-400" />
            </div>
            <div className="text-center">
              <p className="text-base font-bold text-slate-500">No trend snapshots yet</p>
              <p className="text-xs text-slate-400 mt-1">Will populate after daily health snapshot job runs</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-6 mt-4 text-xs font-bold text-slate-400">
        <span className="flex items-center gap-2"><span className="w-5 h-0.5 inline-block rounded bg-sky-500" /> Avg Health Score</span>
        <span className="flex items-center gap-2"><span className="w-5 h-0.5 inline-block border-t-2 border-dashed border-teal-600" /> Daily Engagement</span>
      </div>
    </div>
  );
}
...
function SlideAttention({ data }: { data: PortfolioAnalyticsBody }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mb-5">
        <h2 className="text-2xl font-black text-slate-800 flex items-center gap-3">
          <div className="p-2 rounded-xl border border-slate-100 bg-amber-50">
      ...
      {critical.length > 0 ? (
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 overflow-auto">
          {critical.map((acc) => {
            ...
            const accentColor = isCritical ? '#be123c' : '#b45309';
            const accentBg = isCritical ? 'rgba(244,63,94,0.06)' : 'rgba(245,158,11,0.06)';
            const accentBorder = isCritical ? 'rgba(244,63,94,0.18)' : 'rgba(245,158,11,0.18)';
            return (
              <div key={acc.organizationId} className="rounded-2xl p-4 border bg-white border-slate-200 shadow-sm flex flex-col justify-between">
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="min-w-0">
                    <div className="font-bold text-slate-800 truncate">{acc.organizationName || 'Unnamed org'}</div>
                    <div className="text-[10px] font-semibold text-slate-400 mt-0.5 truncate">{acc.accountNumber || acc.organizationId}</div>
                  </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {(acc.openAlertsCritical ?? 0) > 0 && (
                    <span className="flex items-center gap-0.5 px-2 py-0.5 rounded-lg text-[9px] font-bold bg-rose-50 text-rose-700 border border-rose-250 border-rose-200">
                      <AlertTriangle className="w-3 h-3 text-rose-600" /> {acc.openAlertsCritical} alerts
                    </span>
                  )}
                  {acc.countryCode && (
                    <span className="text-[9px] font-bold px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded">{acc.countryCode}</span>
                  )}
                </div>
                {acc.modulesUsed?.length > 0 && (
                  <div className="mt-2.5 text-[10px] font-medium text-slate-400 truncate">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          ...
          <div className="text-center">
            <p className="text-lg font-bold text-emerald-800">All clear!</p>
            <p className="text-sm text-slate-500 mt-1 font-medium">No critical or at-risk accounts — great work by the CS team.</p>
          </div>
        </div>
      )}
    </div>
...
function SlideModules({ data }: { data: PortfolioAnalyticsBody }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col justify-center gap-3 bg-slate-50/50 border border-slate-100 rounded-2xl p-5">
        {barData.map((m) => (
          <div key={m.name}>
            <div className="flex items-center justify-between text-xs font-bold mb-1.5">
              <span className="text-slate-700 truncate max-w-[60%]">{m.name}</span>
              <div className="flex items-center gap-3">
                <span className="text-slate-400 text-xs font-medium">{m.orgCount} accounts</span>
                <span className="font-extrabold text-base" style={{ color: m.color, minWidth: 42, textAlign: 'right' }}>{m.adoption}%</span>
              </div>
            </div>
            <div className="h-2 rounded-full overflow-hidden bg-slate-200/50">
              <div className="h-full rounded-full transition-all duration-1000"
      ...
      {modules.length === 0 && (
        <div className="flex-1 flex items-center justify-center bg-slate-50/50 border border-slate-100 rounded-2xl">
          <p className="text-slate-400 italic text-xs">No module usage data found</p>
        </div>
      )}
    </div>
  );
...
function SlideNextSteps({ data }: { data: PortfolioAnalyticsBody }) {
  ...
  const actions = [
    ...
    },
    {
      ...
      bg: 'rgba(245,158,11,0.06)',
      border: 'rgba(245,158,11,0.18)',
      text: `Re-engage ${s.churnRiskOrgs} org${s.churnRiskOrgs !== 1 ? 's' : ''} with 30+ days of user inactivity (IR-4)`,
    ...
    },
    {
      ...
      text: 'Coach underutilised modules to deepen core breadth (BOQ, Procurement, Schedule) and improve depth records (MD-3)',
      show: true,
    },
    {
      priority: 'Growth',
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col justify-center gap-3 bg-slate-50/55 border border-slate-100 rounded-2xl p-5">
        {actions.map((a, i) => (
          <div key={i} className="flex items-start gap-4 p-3 rounded-xl border bg-white"
            style={{ borderColor: a.border }}>
            <span className="px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider shrink-0 border"
              style={{ background: a.bg, color: a.color, borderColor: a.border }}>
              {a.priority}
            </span>
            <p className="text-slate-700 text-xs font-semibold leading-relaxed">{a.text}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-5 pt-4 border-t border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500 font-bold">Data from IntoAEC CS Hub · <span className="text-slate-400 font-medium">Updated live</span></span>
        </div>
        <span className="text-[10px] font-bold text-slate-400 uppercase">Confidential — CEO & CS Review</span>
      </div>
    </div>
  );
}
...
export const CeoPresentationModal: React.FC<CeoPresentationProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-slate-50 animate-fade-in">
      {/* Top bar */}
      <div className="flex items-center justify-between px-6 py-3 border-b shrink-0 bg-white border-slate-200 shadow-sm">
        {/* Brand + slide info */}
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-800">IntoAEC CS Hub — CEO Presentation</p>
            ...
            <button key={s.id} onClick={() => { setSlide(i); setProgress(0); }}
              ...
              <div className="rounded-full transition-all duration-300"
                style={{
                  width: i === slide ? 24 : 8,
                  height: 8,
                  background: i === slide ? '#0ea5e9' : i < slide ? 'rgba(14,165,233,0.3)' : 'rgba(226,232,240,1)',
                  border: i === slide ? 'none' : '1px solid #cbd5e1',
                }} />
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={autoPlay
              ? { background: 'rgba(14,165,233,0.06)', color: '#0284c7', borderColor: 'rgba(14,165,233,0.2)' }
          ...
          <button onClick={onClose}
            className="p-2 rounded-lg border border-slate-200 bg-slate-50 text-slate-500 hover:text-slate-800 transition-all cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>
      ...
      {autoPlay && (
        <div className="h-0.5 shrink-0 bg-slate-100">
          <div className="h-full transition-none bg-sky-500" style={{ width: `${progress}%` }} />
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Sidebar */}
        <div className="hidden lg:flex flex-col w-52 shrink-0 border-r py-4 px-3 gap-1 bg-white border-slate-200">
          {slides.map((s, i) => {
            const Icon = s.icon;
            const isActive = i === slide;
            return (
              <button key={s.id} onClick={() => { setSlide(i); setProgress(0); }}
                className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-all text-xs font-bold cursor-pointer"
                style={isActive
                  ? { background: 'rgba(14,165,233,0.06)', color: '#0284c7', border: '1px solid rgba(14,165,233,0.18)' }
                  : { color: '#64748b', border: '1px solid transparent' }}>
                <Icon className="w-3.5 h-3.5 shrink-0" />
                <span>{s.label}</span>
                {i < slide && <span className="ml-auto text-[10px] text-emerald-600">✓</span>}
              </button>
            );
          })}
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* Navigation footer */}
          <div className="flex items-center justify-between px-6 py-3 border-t shrink-0 bg-white border-slate-200">
            <button onClick={goPrev} disabled={slide === 0}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-30 border border-slate-200 bg-slate-50 text-slate-600 hover:bg-slate-100 cursor-pointer">
              <ChevronLeft className="w-4 h-4" /> Previous
            </button>

            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-400 font-bold">{slide + 1} / {slides.length}</span>
              <span className="text-xs font-bold text-slate-500">{slides[slide].label}</span>
              <span className="hidden sm:block text-[10px] font-bold text-slate-400">← → to navigate · Space to play</span>
            </div>

            <button onClick={goNext} disabled={slide === slides.length - 1}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-30 border cursor-pointer"
              style={slide < slides.length - 1
                ? { background: 'linear-gradient(135deg,#0ea5e9,#0d9488)', color: '#fff', borderColor: 'transparent', boxShadow: '0 4px 12px rgba(14,165,233,0.15)' }
